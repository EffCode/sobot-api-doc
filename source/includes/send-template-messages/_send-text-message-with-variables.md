...
params | Yes | In case of template body has variables, this JSON array is required. This array consist of JSON array as follows <br/> ```"params":[{ "type": "body",``` <br/> ```"parameters": [{ "type": "text", "text": "8654334" }, ``` <br/> ```{ "type": "text", "text": "Approved" }] } ]``` <br> <br>Make sure that the list of parameters follows the order as per the variables in your template message. <br> If the parameters count does not matched to variable count in template, sending template will be failed.|
